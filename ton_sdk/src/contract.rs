...
use ton_types::{CellData, SliceData, HashmapE};
...
const GAS_PRICE_MC: u128 = 10000;
const GAS_PRICE_WC: u128 = 1000;
...
impl Contract {
    ...
    pub fn local_call(&self, message: TvmMessage) -> SdkResult<(Vec<Message>, u128)> {
        let code = self.code.clone().ok_or(
            SdkError::from(SdkErrorKind::InvalidData("Account has no code".to_owned())))?;

        let mut messages = vec![];

        let (tvm_messages, gas_used) = local_tvm::local_contract_call(
            self.balance,
            self.balance_other_as_hasmape()?,
            &self.id,
            None,
        ...
        let gas_used = <u128>::try_from(gas_used)?;

        let gas_fee = if self.id.get_workchain_id() == -1 {
            gas_used * GAS_PRICE_MC
        } else {
            gas_used * GAS_PRICE_WC
        };
    ...
    fn balance_other_as_hasmape(&self) -> SdkResult<HashmapE> {

...
fn test_constructor_call() {
    ...
    let test_tree = encode_function_call(WALLET_ABI.to_owned(), "constructor".to_owned(), params.to_owned(), None).unwrap();

    let expected_tree = BuilderData::with_bitstring(vec![
        0x00, 0xAC, 0x81, 0x0A, 0x6D, 0x80,
    ]);
...
fn test_signed_call() {
    ...
    let test_tree = encode_function_call(WALLET_ABI.to_owned(), "createLimit".to_owned(), params.to_owned(), Some(&pair)).unwrap();
...
fn test_not_signed_call() {
    ...
    let test_tree = encode_function_call(WALLET_ABI.to_owned(), "getLimitById".to_owned(), params.to_owned(), Some(&pair)).unwrap();

    let expected_tree = BuilderData::with_bitstring(vec![
        0x00, 0xDA, 0x37, 0x46, 0x4F, 0x02, 0x80
    ]);

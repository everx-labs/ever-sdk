use quote::{quote};
use syn::{DeriveInput, Data, DataEnum, Variant, Expr, Lit, Fields};
use api_doc::api;
use crate::utils::{field_from, type_from, field_to_tokens, doc_from};
...
pub fn impl_type_info(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let info = field_from(Some(&input.ident), &input.attrs, ty);
...
fn fields_from(fields: &Fields) -> Vec<api::Field> {
    fields.iter().map(|f| {
        field_from(f.ident.as_ref(), &f.attrs, type_from(&f.ty))
    }).collect()
}

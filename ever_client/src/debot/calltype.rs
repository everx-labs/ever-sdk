...
pub fn prepare_ext_in_message(
    ...
    let ever_client = Arc::new(crate::ClientContext::new(config).unwrap());

    let signer = if let Some(keypair) = keypair {
        let future = crate::crypto::get_signing_box(ever_client.clone(), keypair);
        let signing_box = ever_client.env.block_on(future).unwrap();
        Signer::SigningBox {
    ...
    let future = decode_and_fix_ext_msg(msg, now_ms, &signer, true, &meta, &ever_client);

    let result = ever_client.env.block_on(future);

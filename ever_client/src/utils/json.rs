...
impl JsonHelper for Value {
    fn get_u64(&self, field: &str) -> Result<u64> {
        self[field].as_u64()
            .ok_or_else(|| format_err!(format!("`{}` field must be an unsigned integer", field)))
    }

    fn get_i64(&self, field: &str) -> Result<i64> {
        self[field].as_i64()
            .ok_or_else(|| format_err!(format!("`{}` field must be an integer", field)))
    }

    fn get_str(&self, field: &str) -> Result<&str> {
        self[field].as_str()
            .ok_or_else(|| format_err!(format!("`{}` field must be a string", field)))
    }

    fn get_array(&self, field: &str) -> Result<&Vec<Value>> {
        self[field].as_array()
            .ok_or_else(|| format_err!(format!("`{}` field must be an array", field)))
    }

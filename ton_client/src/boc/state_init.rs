...
pub async fn decode_state_init(
    ...
    let state_init = deserialize_object_from_boc::<StateInit>(&context, &params.state_init, "TVC").await?;
    ...
    let code_depth = state_init.object.code.as_ref().map(|cell| cell.repr_depth() as u32);
    let code_hash = state_init.object.code.as_ref().map(|cell| cell.repr_hash().as_hex_string());
    let compiler_version = state_init.object.code.clone()
    ...
    let data_depth = state_init.object.data.as_ref().map(|cell| cell.repr_depth() as u32);
    let data_hash = state_init.object.data.as_ref().map(|cell| cell.repr_hash().as_hex_string());
    let data = serialize("data", state_init.object.data, params.boc_cache.clone()).await?;

    let library = serialize("library", state_init.object.library.root().cloned(), params.boc_cache.clone()).await?;

...
pub(crate) fn register(handlers: &mut DispatchTable) {
    ...
    handlers.spawn("crypto.math.factorize", |context: &mut ClientContext, hex: String| {
    ...
    handlers.spawn("crypto.math.modularPower", |context: &mut ClientContext, params: ModularPowerParams| {
        api::math::modular_power(&params.base, &params.exponent, &params.modulus)
    });

    // Random

    handlers.call("crypto.random.generateBytes", |context: &mut ClientContext, params: GenerateParams| {
        params.outputEncoding.encode(api::random::generate_bytes(params.length))
    });

    // Keys

    handlers.call_no_args("crypto.ed25519.keypair", |context: &mut ClientContext|
        api::ed25519::generate_keypair());
    handlers.call("crypto.keystore.add", |context: &mut ClientContext, keys: KeyPair| {
        Ok(KeyStore::add(&keys))
    });
    handlers.call("crypto.keystore.remove", |context: &mut ClientContext, handle: String| {
        KeyStore::remove(&handle);
        Ok(())
    });
    handlers.call_no_args("crypto.keystore.clear", |context: &mut ClientContext| {
        KeyStore::clear();
        Ok(())
    });
    ...
    handlers.spawn("crypto.sha256", |context: &mut ClientContext, params: ShaParams| {
        params.outputEncoding.encode(api::sha::sha256(&params.message.decode()?))
    });

    handlers.spawn("crypto.sha512", |context: &mut ClientContext, params: ShaParams| {
        params.outputEncoding.encode(api::sha::sha512(&params.message.decode()?))
    });

    // Scrypt

    handlers.spawn("crypto.scrypt", |context: &mut ClientContext, params: ScryptParams| {
    ...
    handlers.call_no_args("crypto.nacl.box.keypair", |context: &mut ClientContext|
        api::nacl::box_keypair(),
    );
    handlers.call("crypto.nacl.box.keypair.fromSecretKey", |context: &mut ClientContext, secret: String| {
        api::nacl::box_keypair_from_secret_key(&secret)
    });
    handlers.call_no_args("crypto.nacl.sign.keypair", |context: &mut ClientContext|
        api::nacl::sign_keypair(),
    );
    handlers.call("crypto.nacl.sign.keypair.fromSecretKey", |context: &mut ClientContext, secret: String| {
        api::nacl::sign_keypair_from_secret_key(&secret)
    });
    handlers.spawn("crypto.nacl.box", |context: &mut ClientContext, params: NaclBoxParams| {
    ...
    handlers.spawn("crypto.nacl.box.open", |context: &mut ClientContext, params: NaclBoxParams| {
    ...
    handlers.spawn("crypto.nacl.secret.box", |context: &mut ClientContext, params: NaclSecretBoxParams| {
    ...
    handlers.spawn("crypto.nacl.secret.box.open", |context: &mut ClientContext, params: NaclSecretBoxParams| {
    ...
    handlers.spawn("crypto.nacl.sign", |context: &mut ClientContext, params: NaclSignParams| {
        params.outputEncoding.encode(api::nacl::sign(
            params.message.decode()?,
            KeyStore::decode_secret(&params.key, &params.keystoreHandle)?,
        )?)
    });
    handlers.spawn("crypto.nacl.sign.open", |context: &mut ClientContext, params: NaclSignParams| {
        params.outputEncoding.encode(api::nacl::sign_open(
            params.message.decode()?,
            KeyStore::decode_secret(&params.key, &params.keystoreHandle)?,
        )?)
    });
    handlers.spawn("crypto.nacl.sign.detached", |context: &mut ClientContext, params: NaclSignParams| {
    ...
    handlers.spawn_no_args("crypto.mnemonic.words", |context: &mut ClientContext|
        api::mnemonic::mnemonic_get_words(),
    );

    handlers.spawn_no_args("crypto.mnemonic.from.random", |context: &mut ClientContext|
        api::mnemonic::mnemonic_generate_random(),
    );

    handlers.spawn("crypto.mnemonic.from.entropy", |context: &mut ClientContext, params: MnemonicFromEntropyParams| {
        api::mnemonic::mnemonic_from_entropy(&params.entropy.decode()?)
    });

    handlers.spawn("crypto.mnemonic.verify", |context: &mut ClientContext, params: MnemonicVerifyParams| {
    ...
    handlers.spawn("crypto.hdkey.xprv.from.mnemonic", |context: &mut ClientContext, params: HDKeyFromMnemonicParams| {
        api::hdkey::hdkey_xprv_from_mnemonic(&params.phrase)
    });

    handlers.spawn("crypto.hdkey.xprv.derive", |context: &mut ClientContext, params: HDKeyDeriveParams| {
    ...
    handlers.spawn("crypto.hdkey.xprv.derive.path", |context: &mut ClientContext, params: HDKeyDerivePathParams| {
    ...
    handlers.spawn("crypto.hdkey.xprv.secret", |context: &mut ClientContext, params: HDKeyGetKeyParams| {
        api::hdkey::hdkey_secret_from_xprv(&params.serialized)
    });

    handlers.spawn("crypto.hdkey.xprv.public", |context: &mut ClientContext, params: HDKeyGetKeyParams| {

...
fn invalid_msg_error(e: impl Display) -> ClientError {
    Error::invalid_msg(e)
}
...
async fn decode_and_fix_ext_msg(
    ...
    let debot_abi = Contract::load(debot_abi.as_bytes()).map_err(invalid_msg_error)?;
    let mut message: Message = deserialize_object_from_base64(&msg, "message")
        .map_err(invalid_msg_error)?
        .object;
    ...
    let sign_bit = in_body_slice.get_next_bit().map_err(invalid_msg_error)?;
    if let Signer::SigningBox { handle: _ } = signer {
        if !sign_bit {
            return Err(invalid_msg_error("signature bit is zero"));
        }
        in_body_slice
            .get_next_bits(512)
            .map_err(invalid_msg_error)?;
    ...
    in_body_slice.get_next_u64().map_err(invalid_msg_error)?;
    // `expire` is a callback id of debot
    let mut answer_id = in_body_slice.get_next_u32().map_err(invalid_msg_error)?;
    // remember function id
    let mut func_id = in_body_slice.get_next_u32().map_err(invalid_msg_error)?;
    ...
    if result.is_err() {
        ...
        let pubkey_bit = in_body_slice.get_next_bit().map_err(invalid_msg_error)?;
        ...
        in_body_slice.get_next_u64().map_err(invalid_msg_error)?;
        answer_id = in_body_slice.get_next_u32().map_err(invalid_msg_error)?;
        func_id = in_body_slice.get_next_u32().map_err(invalid_msg_error)?;
    ...
    if pubkey_bit_present {
        if let Some(key) = pubkey {
            new_body.append_bit_one().map_err(invalid_msg_error)?;
            new_body
                .append_raw(&hex::decode(&key).unwrap(), 256)
                .map_err(invalid_msg_error)?;
        } else {
            // pubkey bit = 0
            new_body.append_bit_zero().map_err(invalid_msg_error)?;
        }
    }
    ...
    match signer {
        Signer::SigningBox { handle: _ } => {
            ...
            if let Some(signature) = signature {
                ...
                signed_body.append_bit_zero().map_err(invalid_msg_error)?;
            }
        }
        _ => {
            signed_body.append_bit_zero().map_err(invalid_msg_error)?;
...
fn build_answer_msg(
    ...
    let mut new_body = BuilderData::new();
    new_body.append_u32(answer_id).map_err(invalid_msg_error)?;

    if let Some(body_slice) = out_body.as_mut() {
        let response_id = body_slice.get_next_u32().map_err(invalid_msg_error)?;
        let request_id = response_id & !(1u32 << 31);
        if func_id != request_id {
            return Err(invalid_msg_error("incorrect response id"));
        }
        new_body

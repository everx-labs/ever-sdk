...
async fn test_run_executor() {
    let run = |client: Arc<TestClient>, message: ResultOfEncodeMessage, abi: Abi, account: String| async move {
...
async fn test_run_tvm() {
    let run = |client: Arc<TestClient>, message: ResultOfEncodeMessage, abi: Abi, account: String| async move {
        let run_tvm = client.wrap_async(
            run_tvm,
            TvmModule::api(),
            run_message::run_tvm_api(),
        );
        let result = run_tvm
            .call(ParamsOfRunTvm {
                message: message.message,
...
async fn test_run_message<F>(run: impl FnOnce(Arc<TestClient>, ResultOfEncodeMessage, Abi, String) -> F)
where

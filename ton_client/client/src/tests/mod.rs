...
use crate::crypto::keys::{
    account_decode, account_encode_ex, AccountAddressType, Base64AddressParams,
};
...
impl TestClient {
    fn new() -> Self {
        Self::new_with_config(json!({ "baseUrl": get_network_address() }))
    }
    ...
    fn new_with_config(config: Value) -> Self {
        ...
        unsafe { context = tc_create_context() }
    ...
    fn request(&self, method_name: &str, params: Value) -> Result<String, String> {
        unsafe {
...
fn test_tvm_get() {
    ...
    let result = parse_object(client.request(
        "tvm.get",
        json!({
...
fn test_encryption() {
    ...
    let encrypted = parse_object(client.request(
        "crypto.chacha20",
        json!({
    ...
    let decrypted = parse_object(client.request(
        "crypto.chacha20",
        json!({
...
fn test_tg_mnemonic() {
    ...
    let phrase = parse_string(client.request(
    ...
    let mnemonic = parse_string(client.request(
    ...
    let public = get_map_string(
        &parse_object(client.request(
            "crypto.mnemonic.derive.sign.keys",
            json!({
                "phrase": mnemonic,
                "dictionary": 0,
    ...
    let ton_public =
        parse_string(client.request("crypto.ton_public_key_string", Value::String(public)));
    assert_eq!(
        ton_public,
...
fn test_wallet_deploy() {
    ...
    let deployed = parse_object(client.request(
        "contracts.deploy",
        json!({
            "abi": abi.clone(),
            "constructorParams": json!({}),
...
fn test_address_parsing() {
    ...
    let address =
        ton_block::MsgAddressInt::with_standart(None, -1, hex::decode(short).unwrap().into())
            .unwrap();
    let wc0_address =
        ton_block::MsgAddressInt::with_standart(None, 0, hex::decode(short).unwrap().into())
            .unwrap();

    assert_eq!(
        wc0_address,
        account_decode(short).expect("Couldn't parse short address")
    );
    assert_eq!(
        address,
        account_decode(full_std).expect("Couldn't parse full_std address")
    );
    assert_eq!(
        address,
        account_decode(base64).expect("Couldn't parse base64 address")
    );
    assert_eq!(
        address,
        account_decode(base64_url).expect("Couldn't parse base64_url address")
    );
...
fn test_parallel_requests() {
    ...
    client2
        .request("crypto.ed25519.keypair", json!({}))
        .unwrap();

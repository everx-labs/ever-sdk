...
fn js_value_to_value(js_value: JsValue) -> ClientResult<serde_json::Value> {
    js_value.into_serde::<serde_json::Value>()
...
impl ClientEnv {
    ...
    pub async fn set_timer_internal(ms: u64) -> ClientResult<()> {
        let window = web_sys::window()
            .ok_or_else(|| Error::set_timer_error("Can not get `window`"))?;
    ...
    pub async fn websocket_connect_internal(
        ...
        // Connect to a server
        let ws = if let Some(Some(protocols)) = headers.map(|mut headers| headers.remove("Sec-WebSocket-Protocol")) {
        ...
        let (send_sink, mut send_stream) = futures::channel::mpsc::channel::<(String, tokio::sync::oneshot::Sender<ClientResult<()>>)>(100);
        wasm_bindgen_futures::spawn_local(async move {
            log::trace!("Start websocket sending loop");
            while let Some((string, sender)) = send_stream.next().await {
                log::trace!("Websocket send: {}", string);
                let result = ws.send_with_str(&string)
        ...
        let send_sink = futures::sink::drain()
            ...
            .with(move |string: String| {
                ...
                async move {
                    ...
                    send_sink.send((string, sender))
                        .await
                        .map_err(|err| Error::websocket_send_error(
                            format!("can not send data to websocket sending task: {}", err)))?;
                    // receive sending result
                    receiver
                        .await
                        .map_err(|err| Error::websocket_send_error(
                            format!("can not receive result from websocket sending task: {}", err)))?
            }});
        ...
        let resp_value = JsFuture::from(window.fetch_with_request(&request))
            .map(|result| {
                match result {
                    Ok(result) => Ok(result),
                    Err(err) => {
                        Err(Error::http_request_send_error(js_value_to_string(err)))
                    }
                }
            })
            .await?;

...
fn encode_v2() {
    ...
    let deploy_params = |signing: MessageSigning| ParamsOfEncodeMessage {
        abi: abi.clone(),
        address: None,
    ...
    let unsigned: ResultOfEncodeMessage = client.request(
        "abi.encode_message",
        deploy_params(MessageSigning::External(keys.public.clone())),
    );
    ...
    let signed: ResultOfEncodeMessage = client.request(
        "abi.encode_message",
        deploy_params(MessageSigning::Keys(keys.clone())),
    );
    assert_eq!(signed.message, "");

    let address = "0:05beb555e942fa744fd96f45a9ea9d0a8248208ca12421947c06e59bc997d309";
    let run_params = |signing: MessageSigning| ParamsOfEncodeMessage {
    ...
    let unsigned: ResultOfEncodeMessage = client.request(
        "abi.encode_message",
        run_params(MessageSigning::External(keys.public.clone())),
    );
    ...
    let signed: ResultOfEncodeMessage = client.request(
        "abi.encode_message",
        run_params(MessageSigning::Keys(keys.clone())),
    );
    assert_eq!(signed.message, "");
}

...
pub async fn find_last_shard_block(
    context: &Arc<ClientContext>, address: &MsgAddressInt
        ...
        if blocks[0].is_null() {
            ...
            if blocks[0].is_null() {
                return Err(Error::block_not_found(
                    format!("No blocks for workchain {} found", workchain)));
            }
            ...
            blocks[0]["id"]
                .as_str()
                .map(|val| val.to_owned().into())
                .ok_or(Error::block_not_found("No starting Node SE block found".to_owned()))
        } else {
            let shards = blocks[0]["master"]["shard_hashes"]
                .as_array()
                .ok_or(Error::invalid_data("No `shard_hashes` field in masterchain block"))?;

            let shard_block = ton_sdk::Contract::find_matching_shard(shards, address)
                .map_err(|err| Error::invalid_data(
                    format!("find matching shard failed {}", err)
                ))?;
            if shard_block.is_null() {
                return Err(Error::invalid_data(
                    format!("No matching shard for account {} in block {}", address, blocks[0]["id"])));
            }

...
In fact, keys are optional for deploy, but, if you want to be the contract owner then specify Signer object for that. [Read more about Signer object in Reference](../../docs/modules/mod_abi.md#Signer).
...
It, together with its parameters, is specified in the [CallSet](../../docs/modules/mod_abi.md#callset) structure:
...
* You can use SDK function [get_code_from_tvc](../../docs/modules/mod_boc.md#get_code_from_tvc) of `boc` module and retrieve the code's boc from tvc file. 
* You can download another account with the same `code_hash`'s `boc` from graphql, using [query_collection](../queries_and_subscriptions/query_collection.md) method and parse it with [parse_account](../../docs/modules/mod_boc.md#parse_account) function to retrieve the 'code' from the result (function will return the same JSON account object as graphql). 

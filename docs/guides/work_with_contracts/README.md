# Work with contracts

This section contains the following guides on working with contracts:

* [Add Contract to your App](add\_contract\_to\_your\_app.md)
* [Using custom Giver](custom\_giver.md)
* [Deploy](deploy.md)
* [Run on-chain](run\_onchain.md)
* [Run ABI Get Method](run\_abi\_get\_method.md)
* [Run Fift Get Method](run\_fift\_get\_method.md)
* [Message Expiration](../configuration/message\_expiration.md)
* [Work with Events](work\_with\_events.md)
* [Retry Message](../configuration/retry\_message.md)
* [External Signing](external\_signing.md)
* [Emulate Transaction](emulate\_transaction.md)
* [Decode Message](decode\_message.md)
* [Estimate Fees](estimate\_fees.md)
* [Validate address, convert address](validate\_address\_convert\_address.md)
